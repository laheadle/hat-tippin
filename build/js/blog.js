...
/* (DEFVAR *POSTS* ([])) */
var POSTS = [];
/* (DEFUN PUSH-POST (POST) (SETF (AREF *POSTS* (@ *POSTS* LENGTH)) POST)) */
function pushPost(post) {
    return POSTS[POSTS.length] = post;
};
/* (DEFCLASS BLOG-INDEX NIL (DEFUN TABLE-OF-CONTENTS () (DOM (DIV)))
             (DEFUN GET-INITIAL-STATE () (CREATE POSTS *POSTS*))
             (DEFUN RENDER ()
               (DOM
                (DIV (DIV (TABLE-OF-CONTENTS)) (DIV (@ THIS STATE POSTS)))))) */
var blogIndex = React.createClass({ tableOfContents : function () {
    ...
    return { posts : POSTS };
                                    ...
                                    render : function () {
    var g1240 = this;
    return React.DOM.div({  }, [React.DOM.div({  }, [g1240.tableOfContents.bind(g1240)()]), React.DOM.div({  }, [g1240.state.posts])]);
}
                                  });
/* (DEFUN MAKE-INSTANCE (CLAZZ PROPS)
     (FUNCALL (CHAIN *REACT (CREATE-FACTORY CLAZZ PROPS)))) */
function makeInstance(clazz, props) {
    return React.createFactory(clazz, props)();
};
...
/* (CHAIN *REACT
    (RENDER
     (MAKE-INSTANCE (CHAIN *REACT-ROUTER *ROUTER)
                    (CREATE CHILDREN
                     (LIST
                      (MAKE-INSTANCE (CHAIN *REACT-ROUTER *ROUTE)
                                     (CREATE PATH / COMPONENT
                                      (AREF *POSTS* 0))))))
     (@ WINDOW DOCUMENT BODY))) */
React.render(makeInstance(ReactRouter.Router, { children : [makeInstance(ReactRouter.Route, { path : '/', component : POSTS[0] })] }), window.document.body);
